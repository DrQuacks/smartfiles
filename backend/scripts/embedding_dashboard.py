...
def peek_embeddings(collection: Any, limit: int) -> Dict[str, Any]:
    ...
    items without requiring us to know their IDs in advance.
    """

    # ``include`` mirrors what we store during indexing.
    result = collection.peek(limit=limit, include=["embeddings", "documents", "metadatas"])
    # Some older versions of Chroma may not support the ``include``
    # argument on ``peek``; fall back to whatever is returned.
    return result
